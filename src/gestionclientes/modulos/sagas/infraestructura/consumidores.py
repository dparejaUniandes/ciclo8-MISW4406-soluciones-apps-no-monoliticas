...
from gestionclientes.modulos.sagas.infraestructura.schema.v1.eventos import (
    EventoNotificacionCreada, EventoPagoConfirmado, EventoPagoRealizado)
...
def suscribirse_a_eventos_notificacion():
    cliente = None
    try:
        cliente = pulsar.Client(f'pulsar://{utils.broker_host()}:6650')
        consumidor = cliente.subscribe('eventos-notificaciones', consumer_type=_pulsar.ConsumerType.Shared,subscription_name='sagas-sub-eventos-notificacion', schema=AvroSchema(EventoNotificacionCreada))

        while True:
            mensaje = consumidor.receive()
            print(f'Evento recibido desde notificación hacia sagas: {mensaje.value().data}')

            data = mensaje.value().data
            event_type = mensaje.value().event_type
            if event_type == "notificacion_creada":
            ...
            elif event_type == "notificacion_revertida":
